...
describe("queueStore", () => {
  ...
  it("queues to bottom", () => {
    ...
    expect(putMock).toHaveBeenCalledWith("queue/state", {
      songs: queueStore.all.map((song) => song.id),
    });
  });
  ...
  it.each<[PlaybackState]>([["Playing"], ["Paused"]])(
    "identifies the current song by %s state",
    (state) => {
