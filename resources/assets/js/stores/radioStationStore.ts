...
export const radioStationStore = {
  ...
  async fetchAll(favoritesOnly = false) {
    return this.sync(
      await http.get<RadioStation[]>(`radio/stations?favorites_only=${favoritesOnly}`),
    );
  },
  ...
  async fetchNowPlaying(station: RadioStation) {
    try {
      const response = await http.get<NowPlayingResponse>(
        `radio/stations/${station.id}/now-playing`,
      );
