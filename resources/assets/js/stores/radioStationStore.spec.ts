...
describe("radioStationStore", () => {
  ...
  it("syncs stations", () => {
    ...
    const updatedStation = h
      .factory("radio-station")
      .make({ id: stations[0].id, name: "Updated Station" });
  ...
  it.each([[true], [false]])(
    "fetches all stations with favorites only set to %s",
    async (favoritesOnly) => {
      ...
      expect(getMock).toHaveBeenCalledWith(
        `radio/stations?favorites_only=${favoritesOnly ? "true" : "false"}`,
      );
  ...
  it("toggles favorite status of a station", async () => {
    ...
    const postMock = h
      .mock(http, "post")
      .mockResolvedValue({ id: station.id, type: "radio-station" });
