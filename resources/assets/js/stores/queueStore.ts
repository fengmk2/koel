...
export const queueStore = {
  ...
  queueIfNotQueued(
    playable: Playable,
    position: "top" | "bottom" | "after-current" = "after-current",
  ) {
  ...
  get current() {
    ...
    return (
      this.all.find(({ playback_state }) => playback_state !== "Stopped") ||
      playableStore.findPlaying()
    );
  },
  ...
  async fetchInOrder(sortField: PlayableListSortField, order: SortOrder, limit = 500) {
    this.all = await http.get<Song[]>(
      `queue/fetch?order=${order}&sort=${sortField}&limit=${limit}`,
    );
    return this.all;
  },
