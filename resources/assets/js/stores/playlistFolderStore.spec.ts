...
describe("playlistFolderStore", () => {
  ...
  it("adds a playlist to folder", async () => {
    ...
    expect(http.post).toHaveBeenCalledWith(`playlist-folders/${folder.id}/playlists`, {
  ...
  it("removes a playlist from folder", async () => {
    ...
    expect(playlist.folder_id).toBeNull();
    expect(http.delete).toHaveBeenCalledWith(`playlist-folders/${folder.id}/playlists`, {
      playlists: [playlist.id],
    });
  });
