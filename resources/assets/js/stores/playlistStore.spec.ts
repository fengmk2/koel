...
describe("playlistStore", () => {
  ...
  it("serializes playlist for storage", () => {
    expect(playlistStore.serializeSmartPlaylistRulesForStorage(ruleGroups)).toEqual(
  ...
  it("updates a smart playlist", async () => {
    ...
    const serializeMock = h.mock(playlistStore, "serializeSmartPlaylistRulesForStorage", [
      "Whatever",
    ]);
    const putMock = h.mock(http, "put").mockResolvedValue(playlist);
    const removeMock = h.mock(cache, "remove");
