...
describe("playableStore", () => {
  ...
  it("gets formatted length", () => {
    expect(playableStore.getFormattedLength(h.factory("song").make({ length: 123 }))).toBe(
      "2 min 3 sec",
    );
    expect(
  ...
  it("syncs artist properties", () => {
    ...
    songsContributedByArtist.forEach(({ album_artist_name }) =>
      expect(album_artist_name).toBe("New Artist Name"),
    );
  });
});
