...
export const overviewStore = {
  ...
  async fetch() {
    ...
    this.state.recentlyAddedSongs = playableStore.syncWithVault(
      resource.recently_added_songs,
    ) as Song[];
    this.state.leastPlayedSongs = playableStore.syncWithVault(
      resource.least_played_songs,
    ) as Song[];
    ...
    recentlyPlayedStore.excerptState.playables = playableStore.syncWithVault(
      resource.recently_played_songs,
    );
  ...
  async refreshRandomSongs() {
    const songs = playableStore.syncWithVault(
