...
export const playableStore = {
  ...
  getFormattedLength: (playables: MaybeArray<Playable>) =>
    secondsToHumanReadable(sumBy(arrayify(playables), "length")),
  ...
  registerPlay: async (playable: Playable) => {
    const interaction = await http.silently.post<Interaction>("interaction/play", {
      song: playable.id,
    });
  ...
  async paginateSongs(params: SongListPaginateParams) {
    const resource = await http.get<PaginatorResource<Playable>>(
  ...
  async resolveSongsFromMediaReferences(data: MediaReference[], shuffle = false) {
    const songReferences = data.filter((item) => item.type === "songs") as Array<
      Pick<Song, "type" | "id">
    >;
    const songs = this.byIds(songReferences.map((item) => item.id)) as Song[];

    const folderReferences = data.filter((item) => item.type === "folders") as Array<
      Pick<Folder, "type" | "path">
    >;
