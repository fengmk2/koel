...
const [selectedPlayables, setSelectedPlayables] =
  requireInjection<[Ref<Playable[]>, Closure]>(SelectedPlayablesKey);
const [sortField] =
...
const contentType = computed(() =>
  getPlayableCollectionContentType(rows.value.map(({ playable }) => playable)),
);

const getAllPlayablesWithSort = () => rows.value.map((row) => row.playable);

watch(selected, () => setSelectedPlayables(selected.value.map(({ playable }) => playable)), {
  deep: true,
});
...
const onDragOver = throttle((event: DragEvent) => {
  ...
  if (acceptsDrop(event)) {
    ...
    target.classList.add(
      "droppable",
      event.clientY < midPoint ? "dragover-top" : "dragover-bottom",
    );
  }
...
const noDiscLabel = computed(
  () => noOrOneDiscOnly.value || !sortingByTrack.value || !inAlbumContext.value,
);
