...
describe("playableListHeaderActionMenu.vue", () => {
  ...
  it("contains proper items for song-only lists", () => {
    ...
    ["Title", "Album", "Artist", "Track & Disc", "Time", "Date Added"].forEach((text) =>
  ...
  it("contains proper items for mixed-content lists", () => {
    ...
    ["Title", "Album or Podcast", "Artist or Author", "Date Added"].forEach((text) =>
      screen.getByText(text),
    );
    ["Album", "Artist", "Podcast", "Author"].forEach((text) =>
      expect(screen.queryByText(text)).toBeNull(),
    );
  });
  ...
  it("has toggleable checkboxes for collaborative columns", async () => {
    ...
    ["User", "Contributed"].forEach((text) =>
      screen.getByTitle(`Click to toggle the ${text} column`),
    );
  });
  ...
  it("has a checkbox to toggle the column visibility", async () => {
    ...
    ["Album", "Track & Disc", "Time"].forEach((text) =>
      screen.getByTitle(`Click to toggle the ${text} column`),
    );
