...
describe("breadcrumbs.vue", () => {
  ...
  it("renders the full ancestor chain when within the limit", () => {
    ...
    expect(screen.getByRole("link", { name: "Music" }).getAttribute("href")).toBe(
      `/#/browse/${music.id}`,
    );
    expect(screen.getByRole("link", { name: "Rock" }).getAttribute("href")).toBe(
      `/#/browse/${rock.id}`,
    );
  ...
  it("truncates the middle when the chain is deep", () => {
    ...
    screen.getByRole("link", { name: "Library" });
    screen.getByText("…");
    expect(screen.getByRole("link", { name: "Pink Floyd" }).getAttribute("href")).toBe(
      `/#/browse/${ancestors[3].id}`,
    );
