...
const singleFolderSelected = computed(
  () => selectedItems.value.length === 1 && !isSong(selectedItems.value[0]),
);
...
const handleEnter = async (event: KeyboardEvent) => {
  ...
  const resolvedSongs = onlySongsSelected.value
    ...
    : await playableStore.resolveSongsFromMediaReferences(
        mediaBrowser.extractMediaReferences(selectedItems.value),
      );
