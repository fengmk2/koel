  ...
  <form class="max-w-[540px]" @submit.prevent="handleSubmit" @keydown.esc="maybeClose">
    <header class="gap-4">
      <img
        :src="coverUrl"
        alt=""
        class="w-[84px] aspect-square object-cover object-center rounded-md"
      />
...
if (
  ...
  songs[0].album_artist_id === songs[0].artist_id
) {
  ...
  initialValues.album_artist_name = allSongsShareSameValue("album_artist_name")
    ? songs[0].album_artist_name
    : "";
}
...
const displayedTitle = computed(() =>
  editingOnlyOneSong ? data.title : `${songs.length} songs selected`,
);
