<template>
  <article :title="file.message" class="upload-item relative">
    <div
      :class="cssClass"
      class="h-full w-full min-h-[32px] bg-k-fg-5 relative rounded-lg overflow-hidden"
    >
...
const canRetry = computed(
  () => file.value.status === "Canceled" || file.value.status === "Errored",
);
...
const progressBarWidth = computed(() =>
  file.value.status === "Uploading" ? `${file.value.progress}%` : "0",
);
