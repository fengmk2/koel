...
const totalHeight = computed(() =>
  rowCount.value
    ? rowCount.value * rowHeight.value - measuredRowGap.value + measuredPaddingY.value
    : 0,
);

const startRow = computed(() =>
  Math.max(0, Math.floor(scrollTop.value / rowHeight.value) - renderAhead),
);
...
const measure = async () => {
  ...
  measuredPaddingX.value =
    (parseFloat(style.paddingLeft) || 0) + (parseFloat(style.paddingRight) || 0);
  measuredPaddingY.value =
