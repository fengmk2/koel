    ...
    >
      <aside class="hidden md:block md:flex-[0_0_128px]">
        <img
          :src="podcast.image"
          alt="Podcast image"
          class="w-[128px] aspect-square object-cover rounded-lg"
        />
      </aside>
      <main class="flex-1">
        <header>
          ...
          <p class="mt-2">
            ...
            <template v-if="lastPlayedAt">
              ...
              <span class="text-k-fg-50">
                Last played
                <time :datetime="podcast.last_played_at" :title="podcast.last_played_at">{{
                  lastPlayedAt
                }}</time>
              </span>
            </template>
        ...
        <div
          v-koel-new-tab
          class="description mt-3 line-clamp-3 text-k-fg-70"
          v-html="description"
        />
      </main>
    </a>
...
const PodcastContextMenu = defineAsyncComponent(
