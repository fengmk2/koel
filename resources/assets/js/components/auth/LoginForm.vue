  ...
  <div class="flex items-center justify-center min-h-screen my-0 mx-auto flex-col gap-5">
    ...
    >
      ...
      <FormRow v-if="canResetPassword">
        <a
          class="text-right text-[.95rem] text-k-fg-70"
          role="button"
    ...
    >
      <GoogleLoginButton
        v-if="ssoProviders.includes('Google')"
        @error="onSSOError"
        @success="onSSOSuccess"
      />
      <OpenIDLoginButton
        v-if="ssoProviders.includes('OpenID Connect')"
        @error="onSSOError"
        @success="onSSOSuccess"
      />
    </div>
    ...
    <ForgotPasswordForm
      v-if="showingForgotPasswordForm"
      @cancel="showingForgotPasswordForm = false"
    />
  </div>
</template>
