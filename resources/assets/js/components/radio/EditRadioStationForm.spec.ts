...
describe("editRadioStationForm.vue", () => {
  ...
  it("edits a radio station without logo changing", async () => {
    ...
    const { station } = renderComponent(
      reactive(h.factory("radio-station").make({ is_public: false })),
    );

    await h.type(screen.getByPlaceholderText("My Favorite Radio Station"), "Beethoven Goes Pop");
    await h.type(
      screen.getByPlaceholderText("https://radio.example.com/stream"),
      "https://beet.stream/pop",
    );
