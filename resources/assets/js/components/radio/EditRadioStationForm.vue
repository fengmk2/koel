...
const { data, isPristine, handleSubmit } = useForm<RadioStationData>({
  initialValues: {
    ...pick(station, "name", "url", "homepage_url", "description", "is_public", "logo"),
  },
  onSubmit: async (data) => {
    ...
    const onAirUrl =
      current?.id === station.id && current.playback_state === "Playing" ? station.url : null;
