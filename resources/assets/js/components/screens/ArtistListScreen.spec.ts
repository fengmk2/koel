...
const virtualGridStub = {
  template:
    '<div data-testid="artist-list"><slot v-for="(item, i) in items" :key="i" :item="item" /></div>',
  ...
  it("switches layout via view mode toggle", async () => {
    ...
    await waitFor(() => {
      screen
        ...
        .forEach((card: HTMLElement) => expect(card.dataset.layout).toBe("compact"));
    });
    ...
    await waitFor(() => {
      screen
        .getAllByTestId("artist-card")
        .forEach((card: HTMLElement) => expect(card.dataset.layout).toBe("full"));
    });
  });
