<template>
  <ScreenBase>
        ...
        <template #controls>
          <div class="flex gap-2">
            <Btn
              v-koel-tooltip
              :title="
                preferences.radio_stations_favorites_only ? 'Show all' : 'Show favorites only'
              "
    ...
    <div v-else ref="gridContainer" v-koel-overflow-fade class="-m-6 flex-1 overflow-auto">
      <GridListView
        ref="grid"
        :view-mode="preferences.radio_stations_view_mode"
        data-testid="radio-station-grid"
      >
...
const AddRadioStationForm = defineAsyncComponent(
  () => import("@/components/radio/AddRadioStationForm.vue"),
);
const { openModal } = useModal();
const { currentUserCan } = usePolicies();
const fuzzy = useFuzzySearch<RadioStation>(radioStationStore.state.stations, [
  "name",
  "description",
]);
