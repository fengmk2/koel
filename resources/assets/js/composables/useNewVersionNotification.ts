...
export const useNewVersionNotification = () => {
  ...
  const hasNewVersion = computed(() =>
    compareVersions.compare(latestVersion.value, currentVersion.value, ">"),
  );
  const shouldNotifyNewVersion = computed(
    () => currentUserCan.manageSettings() && hasNewVersion.value,
  );
