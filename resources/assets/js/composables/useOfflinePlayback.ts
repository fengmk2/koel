...
const setupSongDeletionListener = () => {
  eventBus.on("SONGS_DELETED", (deletedSongs) => {
    ...
    manifestEntries.value = manifestEntries.value.filter(
      (e) => !deletedSongs.some((s) => s.id === e.playable.id),
    );
  });
};
...
export const useOfflinePlayback = () => {
  ...
  const getCachingProgress = (playable: Playable): number =>
    cachingProgress.value.get(playable.id) ?? 0;
  const hasCachingError = (playable: Playable): boolean => cachingErrors.value.has(playable.id);
  const getCachingError = (playable: Playable): string | undefined =>
    cachingErrors.value.get(playable.id);
