...
export const useListSelection = <T>(
  ...
  const resolveIdPath = (selectable: Selectable<T>) =>
    typeof idPath === "function" ? idPath(selectable) : idPath;
  ...
  const selectBetween = (first: Selectable<T>, second: Selectable<T>) => {
    const firstIndex = Math.max(
      0,
      selectables.value.findIndex(
        (s) => get(s, resolveIdPath(s)) === get(first, resolveIdPath(first)),
      ),
    );
    ...
    const secondIndex = Math.max(
      0,
      selectables.value.findIndex(
        (s) => get(s, resolveIdPath(s)) === get(second, resolveIdPath(second)),
      ),
    );
