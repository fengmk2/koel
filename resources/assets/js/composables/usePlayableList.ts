...
const PlayableList = defineAsyncComponent(
  () => import("@/components/playable/playable-list/PlayableList.vue"),
);
...
export const usePlayableList = (
  ...
  const thumbnails = computed(() => {
    const playablesWithCover = playables.value.filter((p) =>
      getPlayableProp(p, "album_cover", "episode_image"),
    );

    const sampleCovers = playablesWithCover
      .slice(0, 100)
  ...
  const playSelected = (shuffle: boolean) =>
    playback().queueAndPlay(selectedPlayables.value, shuffle);

  const applyFilter = useThrottleFn(
    (keywords: string) => (filterKeywords.value = keywords),
    200,
    true,
  );
  ...
  const onPressEnter = async (event: KeyboardEvent) => {
    ...
    event.shiftKey
      ? queueStore.queueToTop(selectedPlayables.value)
      : queueStore.queue(selectedPlayables.value);
  ...
  const sort = (
    by: MaybeArray<PlayableListSortField> | null = sortField.value,
  ...
  eventBus.on(
    "SONGS_DELETED",
    (deletedSongs) => (playables.value = differenceBy(playables.value, deletedSongs, "id")),
  );
