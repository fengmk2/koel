...
describe("downloadService", () => {
  ...
  it("does not download multiple playables if check fails", async () => {
    const triggerMock = h.mock(downloadService, "trigger");
    h.mock(downloadService, "checkDownloadable").mockRejectedValue(
      new DownloadLimitExceededError("Limit exceeded"),
    );
  ...
  it("throws DownloadLimitExceededError if check fails", async () => {
    ...
    await expect(downloadService.fromAlbum(h.factory("album").make())).rejects.toThrow(
