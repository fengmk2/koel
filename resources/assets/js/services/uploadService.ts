...
export const uploadService = {
  ...
  async upload(file: UploadFile) {
    ...
    const { promise, abort } = postWithProgress<UploadResult | null>(
      "upload",
      formData,
      (e: ProgressEvent) => {
    ...
    try {
      ...
      const message =
        isObjectResponse && "message" in responseData
          ? (responseData as { message?: unknown }).message
          : undefined;

      file.message =
        typeof message === "string" && message ? `Upload failed: ${message}` : "Server error.";
