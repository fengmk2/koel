...
describe("socketService", () => {
  ...
  it("does not init without PUSHER_APP_KEY", async () => {
    Object.defineProperty(window, "PUSHER_APP_KEY", {
      value: "",
      writable: true,
      configurable: true,
    });
  ...
  it("inits with PUSHER_APP_KEY", async () => {
    ...
    Object.defineProperty(window, "PUSHER_APP_CLUSTER", {
      value: "mt1",
      writable: true,
      configurable: true,
    });
  ...
  it("broadcasts events", () => {
    ...
    expect(mockChannel.trigger).toHaveBeenCalledWith(`client-TEST_EVENT.${user.id}`, {
      foo: "bar",
    });
  });
