...
describe("formatters utils", () => {
  it.each([
    ...
    [7547, "02:05:47"],
    [137241, "38:07:21"],
  ])("formats %d seconds to H:i:s", (seconds, formatted) =>
    expect(secondsToHis(seconds)).toBe(formatted),
  );
  ...
  ])("converts <br> tags in %s to line breaks", (input, output) =>
    expect(br2nl(input)).toEqual(output),
  );
  ...
  it.each([
    ...
    ["foo-bar--baz---", "Foo Bar Baz"],
  ])("converts %s to the title counterpart", (slug, title) =>
    expect(slugToTitle(slug)).toEqual(title),
  );
  ...
  it.each([
    ...
    [2, "cat", "cats"],
    [0, "cat", "cats"],
  ])("pluralizes %d %s", (count, noun, plural) =>
    expect(pluralize(count, noun)).toEqual(`${count} ${plural}`),
  );
