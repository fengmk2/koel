...
export const arrayify = <T>(maybeArray: MaybeArray<T>) =>
  Array.isArray(maybeArray) ? maybeArray : [maybeArray];

// @ts-ignore
export const noop = () => {};

export const limitBy = <T>(arr: T[], count: number, offset: number = 0): T[] =>
...
export const provideReadonly = <T>(
  ...
  if (!isObject(value)) {
    ...
    provide(key, [
      deep
        ? (readonly(value) as unknown as DeepReadonly<T>)
        : (shallowReadonly(value) as unknown as Readonly<T>),
      mutator,
    ]);
  }
};
